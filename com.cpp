...
{
  uint8_t dir = 0;//robot->sens > 0 ? 1 : 0; TODO
...
void Comm::sendGo(bool isPurple)
{
  uint8_t data[1];
  uint8_t* dataPtr = &(data[0]);
  dataPtr = writeInt8(dataPtr, isPurple ? 0 : 1); //0 = purple, 1 = red 
...
bool Comm::process_message(uint8_t data[], uint8_t instruction, uint8_t length)
{
  ...
  if (instruction == INSTR_DEST_ADD && length == 9)
  {
    //add a point
    Point p;
    short x, y, theta;
