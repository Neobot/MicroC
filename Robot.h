...
#define ACCELARATION_MAX_EN_REEL_ROT 0.005
#define ACCELARATION_MAX_EN_REEL_LIN 0.001 // en mm/ms² ou pas : 0.003 * PERIODE_ASSERV_MS * PERIODE_ASSERV_MS * COEFF_CONVERTION_PAS_METRE // 1m/s² => 0.001 mm/ms² => 0.001*Te² mm mais comme on travaille en pas on multiplis pas le coeef de correction
#define VITESSE_MAX 1.40 // mm/ms

#define VALEUR_MAX_PWM 65535.0
#define OFFSET 1000.0
#define RATIO_PWM 1.0 - OFFSET / VALEUR_MAX_PWM // pwm = OFFSET + consigne * RATIO_PWM = OFFSET + consigne * (VALEUR_MAX_PWM - OFFSET) / VALEUR_MAX_PWM
...
{
  public:
    Robot(float periodAsserv = 5.0, float x = 0.0, float y = 0.0, float theta = 0.0);
    ...
    void ajoutPoint(float x, float y, bool pointArret = false, int typeDeplacement = 1);
    void flush();
    void stop();
    ...
    void avanceDe(float avance, bool avecFreinage = true); // en mm
    void tourneDe(float rotation, bool avecFreinage = true); // en rad
    void vaEnXY(float x, float y, bool estPointArret = true);
    void passageAuPointSuivant();
